...
- **`job_description`**: store the **full** posting body your pipeline can obtain. Prefer **plain text** (line breaks preserved). If you only have HTML from LinkedIn, strip tags / decode entities server-side before insert or update.
...
Use **Admin → Moderation → Edit description** on each job to paste the full text, or run a one-off SQL / script update against `public.jobs.job_description` with the service role.
