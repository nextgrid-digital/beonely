...
| `VITE_RAZORPAY_KEY_ID`          | Optional. Same value as `RAZORPAY_KEY_ID` (publishable **key id** only). If set, the client uses it for Checkout; if omitted, the [`create-order` handler](../api/_handlers/payments/create-order.ts) still returns `keyId` from the server env so checkout works. **Never** put `RAZORPAY_KEY_SECRET` here or under any `VITE_*` name. |
...
| `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET` | Razorpay **server** credentials for the consolidated [`api/payments.ts`](../api/payments.ts) entrypoint and its [`create-order`](../api/_handlers/payments/create-order.ts) / [`verify-payment`](../api/_handlers/payments/verify-payment.ts) handlers. `KEY_ID` matches `VITE_RAZORPAY_KEY_ID` when both are set. Order amounts must match [`PLAN_AMOUNT_INR_PAISE`](../src/lib/payments/plans.ts) (base + 18% GST). |
...
On Vercel, [`api/payments.ts`](../api/payments.ts) runs as a consolidated **serverless function** on the same origin and preserves `/api/create-order` and `/api/verify-payment` through rewrites — no local `vercel dev` or Vite proxy is involved. Confirm **Production** (and **Preview**, if you test there) has at least:
...
| Required                                         | `RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`, `RAZORPAY_WEBHOOK_SECRET` | Razorpay Standard Checkout and signed, server-to-server fulfillment ([create-order handler](../api/_handlers/payments/create-order.ts), [`api/razorpay-webhook.ts`](../api/razorpay-webhook.ts)). |
...
When misconfiguration is limited to **missing service role or Supabase URL**, the API now responds with **503** and JSON `{ "error": "server_misconfigured", ... }` instead of throwing (see [`tryGetServiceSupabase`](../api/_lib/supabase.ts) in the [`create-order`](../api/_handlers/payments/create-order.ts) and [`verify-payment`](../api/_handlers/payments/verify-payment.ts) handlers). Other startup failures may still surface as HTML until fixed in logs.
