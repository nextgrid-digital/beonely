...
describe('RecordApplicationButton', () => {
  ...
  it('shows disabled state when application already exists', async () => {
    ...
    await vi.waitFor(() =>
      expect.element(screen.getByRole('button', { name: /Application recorded/i })).toBeDisabled()
    )
  })
  ...
  it('saves application and shows success toast', async () => {
    ...
    await vi.waitFor(() =>
      expect.element(screen.getByRole('button', { name: /Mark as applied/i })).toBeInTheDocument()
    )
    ...
    await vi.waitFor(() =>
      expect(vi.mocked(toast.success)).toHaveBeenCalledWith('Application recorded')
    )
    expect(invalidateSpy).toHaveBeenCalled()
  })

  it('shows duplicate error toast on unique violation', async () => {
    insertApp.mockResolvedValue({ error: { code: '23505', message: 'duplicate' } })
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } })
    const screen = await renderRecord(
      <RecordApplicationButton jobId='job-1' jobTitle='Engineer' />,
      qc
